...
def main(config_path):
    ...
    start = time.time()
    print(
        f"[STATUS] Starting cell type HeartNet with config '{config_name}'.", flush=True
    )
    ...
    print(
        f"[STATUS] Converting cell type adjacency matrices into edge lists...",
        flush=True,
    )
    ...
    # Convert adjacency matrices to edgelists
        ...
        for sample in prepro_data_dir.iterdir():
            if sample.is_dir():
                ...
                print(
                    f"[STATUS] Converting cell type adjacency matrix of sample {sample_name}.",
                    flush=True,
                )
                output_file = edgelists_dir.joinpath(sample_name + ".edg")

                data_preprocessing.adjacency_matrices_to_celltype_edgelist(
    ...
    print(
        f"[STATUS] Finished celltype preprocessing for config '{config_name}' after"
    ...
    embeddings_dir = Path(config["data"]["embeddings"]).joinpath(
        f"celltype_{config_name}_{date_time}"
    )
    ...
    gw_dir = Path(config["data"]["gw_dir"]).joinpath(
        f"celltype_{config_name}_{date_time}"
    )
    ...
    rem_minutes = round(minutes % 60, 1)
    print(
        f"[STATUS] Finished cell type HeartNet calculations for config '{config_name}'"
        f" after {hours} hours {rem_minutes} mins.",
        flush=True,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            ...
            " distance calculation, as specified by the config file, only on the cell"
            " type layer."
        )
    )
